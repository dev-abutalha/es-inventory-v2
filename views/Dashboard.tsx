...
const Dashboard = ({ user }: { user: User }) => {
  ...
  const storeSummaries = useMemo(() => {
    const activeStores = isAnyAdmin ? stores.filter(s => s.id !== 'central') : stores.filter(s => s.id === user.assignedStoreId);
    return activeStores.map(store => {
      // Money stats only for Super Admin
      const storeSales = isSuperAdmin ? sales.filter(s => s.storeId === store.id && isWithinRange(s.date)).reduce((acc, s) => acc + s.amount, 0) : 0;
