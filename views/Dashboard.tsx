...
const Dashboard = ({ user }: { user: User }) => {
  ...
  const filteredSales = useMemo(() => {
    const list = isAdmin ? sales : sales.filter(s => s.storeId === user.assignedStoreId);
    return list.filter(s => isWithinRange(s.date));
  }, [sales, isAdmin, user.assignedStoreId, dateFrom, dateTo]);

  const totalSales = useMemo(() => filteredSales.reduce((acc, s) => acc + s.amount, 0), [filteredSales]);
  const totalPurchases = useMemo(() => purchases.filter(p => isWithinRange(p.date)).reduce((acc, p) => acc + p.totalCost, 0), [purchases, dateFrom, dateTo]);
  const totalExpenses = useMemo(() => expenses.filter(e => isWithinRange(e.date)).reduce((acc, e) => acc + e.amount, 0), [expenses, dateFrom, dateTo]);
  const netProfit = totalSales - totalPurchases - totalExpenses;

  const storeSummaries = useMemo(() => {
    const activeStores = isAdmin ? stores.filter(s => s.id !== 'central') : stores.filter(s => s.id === user.assignedStoreId);
    return activeStores.map(store => {
      const storeSales = sales.filter(s => s.storeId === store.id && isWithinRange(s.date)).reduce((acc, s) => acc + s.amount, 0);
      const storeStock = stock.filter(s => s.storeId === store.id).reduce((acc, s) => acc + s.quantity, 0);
  ...
  }, [stores, sales, stock, requests, transfers, isAdmin, user.assignedStoreId, dateFrom, dateTo]);
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {isAdmin && <StatCard title="OpEx Costs" value={`€${totalExpenses.toLocaleString()}`} icon={Receipt} color="rose" />}
        {isAdmin && <StatCard title="Supply Spend" value={`€${totalPurchases.toLocaleString()}`} icon={CreditCard} color="primary" />}
        {isAdmin && <StatCard title="Est. Profit" value={`€${netProfit.toLocaleString()}`} icon={DollarSign} color="indigo" />}
      </div>
              ...
              <div key={store.id} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative group hover:border-primary-200 transition-all">
                {store.pendingReqs > 0 && (
                  <div className="absolute top-6 right-6 flex items-center gap-2 bg-rose-50 text-rose-600 px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-rose-100 animate-bounce">
                    <AlertCircle size={12} /> {store.pendingReqs} {isAdmin ? 'New Request' : 'Request Pending'}
                  </div>
                )}
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
            ...
            <div className="space-y-4">
              {filteredSales.slice(0, 10).map(sale => (
                <div key={sale.id} className="flex justify-between items-center p-5 bg-slate-50 rounded-[1.5rem] border border-transparent hover:border-primary-100 transition-all">
                  <div>
                    <p className="text-sm font-black text-slate-900 leading-none mb-2">{stores.find(s => s.id === sale.storeId)?.name}</p>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{sale.date}</span>
                      {sale.receiptImage && <span className="p-1 bg-emerald-50 text-emerald-500 rounded-md"><Check size={8} /></span>}
                    </div>
                  </div>
                  <span className="text-lg font-black text-primary tracking-tighter">€{sale.amount.toLocaleString()}</span>
                </div>
              ))}
              {filteredSales.length === 0 && <p className="text-center py-20 text-slate-400 text-[10px] font-black uppercase tracking-widest italic">No transactions in range</p>}
            </div>
            {filteredSales.length > 0 && (
              <Link to="/sales" className="block mt-8 text-center text-[10px] font-black text-primary uppercase hover:underline">View all sales log</Link>
            )}
          </div>
        </div>
      </div>
